...
module.exports = class Capture {
  ...
  async goto() {
    ...
    await this.getPageHeight();
    await this.getPageWidth();
    const per_scoll_px = 700;
    const scoll_count = Math.ceil(this.info.pageHeight / per_scoll_px);
  ...
  async getPageHeight() {
    this.info.pageHeight = await this.page.evaluate(
      `Math.max(
  ...
  async getPageWidth() {
    this.info.pageWidth = await this.page.evaluate(
      `Math.max(
  ...
  async save() {
    ...
    await this.page.pdf({
      path: this.pdfPath,
      width: this.info.pageWidth,
      height: this.info.pageHeight,
